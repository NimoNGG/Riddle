...
// =============================================
const quizzes = [
    { title: "第一のエニグマ", question: `学舎（まなびや）と学舎を繋ぐ大いなるデッキ。その影となりし場所に目を向けよ。異国の春雨を用いた料理が提供されている。<br>本場料理人が働くその店に、賢人の言葉は隠されている。`, answer: "マシッソヨ", keyword: "天秤は『3204』に眠る" },
    { title: "第二のエニグマ", question: `第三の学び舎にて、一瞬の時を永遠に封じ込める小部屋を探せ。目映い光と共に、君の姿は一枚の紙へと転写される。<br>その小部屋の周辺に、賢人の言葉は縫い付けられている。`, answer: "メモリア", keyword: "管理者を『番人』と呼び<br>声をかけろ" },
    { title: "第三のエニグマ", question: `森の名を冠した建物へ向かい、幻影の世界へ。特殊な眼鏡をかければ、ここではない世界にて遊戯を行えるだろう。<br>幻影世界への案内人が待つその部屋に、次の言葉は記されている。`, answer: "ダイブ", keyword: "自身のことを<br>『革命家』と名乗れ" },
    { title: "第四のエニグマ", question: `再び、第三の学び舎へ。時の旅人たちが集う部屋を探せ。彼らは戦や遺物のみを扱うに非ず、無数の歴史的文献を紐解く。<br>それを現代に紡ぐ資料が数多く示されている部屋に、言葉は刻まれている。`, answer: "クロニクル", keyword: "革命は<br>『正義の下（もと）にあり』" },
    { title: "第五のエニグマ", question: `またもや第三の学び舎にて、無から有を生み出す錬金術師たちの工房を探せ。彼らが操る回転する円卓の上で、土塊は新たな命を吹き込まれる。<br>自らもその工程を体験できるその場所にて、隠されし合言葉は秘められている。`, answer: "クリエイト", keyword: "錠の暗号は『1034』" },
    { title: "第六のエニグマ", question: `改めて大いなるデッキに向かえ。冷えてしまった身体を温める湯気が立ち上る場所がある。黄金の出汁に浸かる数々の宝。<br>疲れを癒し身体の芯から温まることができるその場所に、最後の言葉は眠る。`, answer: "コア", keyword: "『天秤の傾きを等しくする』<br>行動をとれ" }
];
const scenarios = [
...
function initIndexPage() {
    ...
    if (solvedCount >= quizzes.length) {
        ...
        // ★ 光る条件: 第6問クリア済み かつ 最終章がまだアンロックされていない
    ...
    if (userAnswer === correctAnswer) {
        ...
        if (quizId > currentSolvedCount) {
            if (quizId < quizzes.length) {
                // ★★★ 修正 1: 強調表示 ★★★
                resultMessage += `<span class="keyword">また新たなシナリオとエニグマが<br>解放された。</span><br>`;
            } else {
                // ★★★ 修正 1: 強調表示 ★★★
                resultMessage += `<span class="keyword">また新たなシナリオが解放された。</span><br>`;
            }
        ...
        // (ここのメッセージは .next-step-message クラスでCSS側で強調されます)
        if (quizId === quizzes.length) {
